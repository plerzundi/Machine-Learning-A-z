# -*- coding: utf-8 -*-
"""
Created on Wed Oct  6 04:16:37 2021
...
# Ajustar el clasificador de Random Forest Classification en el conjunto de entrenamiento
...
# Prediccion del modelo con el conjunto de testing
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Random Forest Classification (Conjunto de Testing)')
plt.xlabel('Edad')
plt.ylabel('Sueldo estimado')
plt.legend()
plt.show()
